...
created(){
  ...
  this.notasFiltradas = this.tasks;
    ...
    methods: {
      async leerDatos(){
        try {          
          const query = await db.collection('tareas2').get()
      ...
      async saveWork () {
        try {
          const query = await db.collection('tareas2').add({
            texto: this.editor,
            estado: false
          })
          this.tasks.push({
      ...
      async updateWork(){
        try {
          ...
          const query = await db.collection('tareas2').doc(this.id).update({
            texto: this.editor
          })
      ...
      eliminar(index, id){
        this.$q.dialog({
          ...
          try {
            ...
            const query = await db.collection('tareas2').doc(id).delete()
